...
public class PrimitiveStreams
{
   public static void main(String[] args)
   {
      ...
      System.out.printf("The sum of a stream of ints: %d ", stream.sum());

      int[] values = { 3, 1, 4, 1, 5, 9 };
      stream = IntStream.of(values);
      System.out.printf("The sum of a stream of ints from an array: %d", stream.sum());
      ...
      System.out.println("Mapping objects to ints: " 
         + Arrays.toString(stream.toArray()));

      stream = IntStream.range(0, 20).map(n -> Math.max(n, 10));
      System.out.println("Mapping ints to ints: "
         + Arrays.toString(stream.toArray()));

      String river = "Mississippi";
      int n = river.length();
      Stream<String> prefixes
         = IntStream.range(0, n).mapToObj(i -> river.substring(0, i + 1));
      System.out.println("Mapping ints to objects: " 
         + Arrays.toString(prefixes.toArray(String[]::new)));
      
      DoubleStream roots = IntStream.range(0, 20).mapToDouble(i -> Math.sqrt(i));
      System.out.println("Mapping ints to doubles: "
         + Arrays.toString(roots.toArray()));
      ...
      // Get code point for each character of the string
      int max = "World".codePoints()
         .max()
         .orElse(0);
